...
| Value arrays | In scope. Homogeneous one-dimensional arrays, empty arrays, literals, indexing, and replacement. | 6.3.1. Direct arrays of arrays are not standard-valid. Arrays of structs may contain array members. |
...
| Empty arrays | 6.3.1 explicitly permits an empty literal; Annex B's production appears to require an element. Prefer the explicit semantic rule. The tagged compiler grammar accepts the empty form. |
